  ...
  const service: LedgerService = {
    ...
    async getWalletPublicKey(
      ...
      if (!isConnected || !appBTC) {
        throw new Error('Ledger is not connected. Please connect first.');
      }
      ...
      try {
        ...
        throw new Error(
          `Failed to get public key: ${error?.message || error}`,
        );
      }
    },
    ...
    async signTransaction(psbt: bitcoin.Psbt, paths: string[]): Promise<string> {
      ...
      try {
        ...
        while (retryCount <= maxRetries) {
          try {
            // Ensure transport is valid before signing
            if (retryCount > 0) {
        ...
        for (let i = 0; i < psbtClone.inputCount && i < signedTx.ins.length; i++) {
          ...
          if (isTaprootInput) {
            ...
            const isValid = psbtClone.validateSignaturesOfInput(
              i,
              (pubkey, hash, sig) => {
                try {
            ...
            // Signature validation result is ignored
          }
        }
        ...
        const errorMsg = error?.message || String(error);
        if (errorMsg.includes('not supported') || errorMsg.includes('0x6a80')) {
          throw new Error('Ledger app does not support PSBT signing. Please update the Bitcoin app on your Ledger to version 2.1.0 or later.');
        }
    ...
    async signMessage(path: string, message: string): Promise<Uint8Array> {
      ...
      try {
        ...
        // Accept both "m/84'/0'/0'/0/0" and "84'/0'/0'/0/0" forms
        const normalizedPath = path.replace(/^m\//, '');
